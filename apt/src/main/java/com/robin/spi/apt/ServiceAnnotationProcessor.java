...
public class ServiceAnnotationProcessor extends BaseProcessor {
    ...
    private void processAnnotations(RoundEnvironment env) {
        for (Element element : env.getElementsAnnotatedWith(ISpiLoader.class)) {
            ...
            ISpiLoader service = cls.getAnnotation(ISpiLoader.class);
            if (service == null) {
                continue;
            }
            ...
            if (typeMirrors != null && !typeMirrors.isEmpty()) {
                for (TypeMirror mirror : typeMirrors) {
                    ...
                    if (!isConcreteSubType(cls, mirror)) {
                        String msg = cls.className() + "没有实现注解" + ISpiLoader.class.getName()
                        ...
                        for (String key : keys) {
                            if (key.contains(":")) {
                                String msg = String.format("%s: 注解%s的key参数不可包含冒号",
                                        implementationName, ISpiLoader.class.getName());
                                throw new RuntimeException(msg);
                            }
    ...
    private static List<? extends TypeMirror> getInterface(ISpiLoader service) {
        try {
            service.interfaces();
    ...
    public Set<String> getSupportedAnnotationTypes() {
        return new HashSet<>(Collections.singletonList(ISpiLoader.class.getName()));
    }
