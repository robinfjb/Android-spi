...
public class SpiLoaderTransform extends Transform {
    ...
    public void transform(TransformInvocation invocation) {
        SpiLoaderLogger.info(TRANSFORM + "start...");
        ...
        SpiLoaderLogger.info(TRANSFORM + "cost %s ms", System.currentTimeMillis() - ms);
    }
    ...
    private void scanJarFile(File file, Set<String> initClasses) throws IOException {
        ...
        while (entries.hasMoreElements()) {
            ...
            if (name.endsWith(SdkConstants.DOT_CLASS) && name.startsWith(INIT_SERVICE_PATH)) {
                ...
                SpiLoaderLogger.info("    find ServiceInitClass: %s", className);
            }
        }
    }
    ...
    private void generateServiceInitClass(String directory, Set<String> classes) {
        ...
        if (classes.isEmpty()) {
            SpiLoaderLogger.info(GENERATE_INIT + "skipped, no service found");
            return;
        }

        try {
            SpiLoaderLogger.info(GENERATE_INIT + "start...");
            long ms = System.currentTimeMillis();
            ...
            SpiLoaderLogger.info(GENERATE_INIT + "cost %s ms", System.currentTimeMillis() - ms);

        } catch (IOException e) {
            SpiLoaderLogger.fatal(e);
        }
    }
}
